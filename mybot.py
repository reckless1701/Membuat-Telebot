...
myDb=mysql.connector.connect(host='localhost',user='root',database='db_belajarbot-master')
sql=myDb.cursor()
...
class Mybot:
    ...
    def start(message):
        # photo = open('img/annn.jpg', 'rb')
        # myBot.send_photo(message.from_user.id, photo)
        teks = mytoken.SAPA + "\n-- admin & developer @BurhanDS17 - Siswa SMK Taruna Bhakti -- "+"\n" \
